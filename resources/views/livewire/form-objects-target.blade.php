<div>
    <div class="bg-white rounded-lg shadow-md w-full p-3 inline-block mt-4"" >
        <h2>Describa ventas con targeta:</h2>

        <div class="grid grid-cols-4 gap-5">
            <div>
                <!-- <x-jet-label class="mb-2 mt-2" for="object-invoice" value="Factura" /> -->
                <x-jet-input class="block mt-1 w-full" type="text" placeholder="Tipo de targeta" wire:model="name" value="{{ $name }}" />
                <x-jet-input-error for="object-name" class="mt-2" />
            </div>

            <div>
                <!-- <x-jet-label class="mb-2 mt-2" for="object-client" value="Cliente" /> -->
                <x-jet-input class="block mt-1 w-full"  type="text" placeholder="Numero de targeta" wire:model="invoice_number" value="{{ $invoice_number }}" />
    ...
    <div class="max-h-60 overflow-y-auto my-2">
        ...
        <div class="grid grid-cols-4 gap-5 ">
            <div>
                ...
                <input type="text" name="object_type_target[]" value="{{ $object['name'] }}" class="block mt-1 w-full" readonly>
            </div>

            <div>
                <!-- <label for="" class="w-full">cliente</label> -->
                <input type="text" name="object_number_invoice_target[]" value="{{ $object['invoice_number'] }}" class="block mt-1 w-full" readonly>
            </div>

            <div>
                <!-- <label for="" class="w-full">Valor</label> -->
                <input type="text" name="object_cash_target[]" value="{{ $object['value'] }}" class="block mt-1 w-full" readonly>
            </div>

            <span style="height:40px; width:60px;" class="bg-red-600 text-white py-1 px-4 rounded-lg cursor-pointer" wire:click="removeObjectToList('{{ $object['name'] }}','{{ $object['invoice_number'] }}','{{ $object['value'] }}')">
